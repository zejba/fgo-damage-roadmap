...
function CardCardInner(props: CardCardInnerProps) {
	...
	const cardBuffAtomsAtom = useMemo(
		() => splitAtom(cardBuffsAtom, (effect) => effect.id),
		[cardBuffsAtom],
	)
...
export function CardCard(props: CardCardProps) {
	...
	const cardParamsAtom = useMemo(
		() => focusAtom(cardAtom, (optic) => optic.prop('params')),
		[cardAtom],
	)
	const cardBuffsAtom = useMemo(
		() => focusAtom(cardAtom, (optic) => optic.prop('buffs')),
		[cardAtom],
	)
	...
	return (
		...
		>
			<MemoizedCardCardInner
				cardParamsAtom={cardParamsAtom}
				cardBuffsAtom={cardBuffsAtom}
			/>
		</Card>
	)
}
